...
cell_id = 228
#cell_id = 160
f = openmc_fluxes[cell_id][0]
fsd = openmc_fluxes[cell_id][1]
...
rms_err = rms_map[cell_id]
print(np.sqrt(np.sum(rms_err)))

print(np.sum(fsd < 0.1))
print(np.sum(fsd2 < 0.1))

fig, ax = plt.subplots()
ax.loglog(ebins[:-1], f, drawstyle='steps-post')
ax.fill_between(ebins[:-1], (1 - 1*fsd)*f, (1 + 1*fsd)*f, alpha=0.3, color='C0',
                step='post')
ax.loglog(ebins[:-1], f2, drawstyle='steps-post')
...
#fsd_combined = (1 + 1/c_over_e) * np.sqrt(fsd**2 + fsd2**2)
fsd_combined = np.sqrt(fsd**2 + fsd2**2)
