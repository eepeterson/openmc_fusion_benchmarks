...
settings.batches = 25000000
settings.particles = 200
settings.survival_biasing = True
settings.photon_transport = True

# use MCNP DENOVO WW file
wws = openmc.wwinp_to_wws('fns_w_global.wwinp')
...
# photon energy spectrum tally   
cell_filter = openmc.CellFilter([cell5, cell11, cell17])
...
e_min, e_max = 0.3*1e6, 1.6e+07
groups = 100
energies = np.logspace(log10(e_min), log10(e_max), groups + 1)
energy_filter = openmc.EnergyFilter(energies)
particle_p = openmc.ParticleFilter(['photon'])
tally_p = openmc.Tally(name="Photon Spectrum")  
tally_p.filters = [cell_filter,energy_filter,particle_p]
tally_p.scores = ['flux'] 

# Instantiate a Tallies collection and export to XML
tallies = openmc.Tallies([tally_n, tally_p])
